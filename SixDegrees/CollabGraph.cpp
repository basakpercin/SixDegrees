 ...
 **
 **   July-August 2022: Basak Percin
 **       Edited the report_path and get_vertex_neighbors
 **
 **/
...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist &artist) const
{
    ...
    vector<Edge> tempList = graph.at(artist.get_name())->neighbors;

    for (size_t i = 0; i < tempList.size(); i++){
        result.push_back(tempList[i].neighbor);
    }
                                            ...
                                            const Artist &dest) const
{
    ...
    Artist tempArtist = dest;
    while (tempArtist != source) {
        if (!is_vertex(tempArtist)) {
            return empty;
        }
        path.push(tempArtist);
        tempArtist = get_predecessor(tempArtist);
    }
    path.push(tempArtist);
